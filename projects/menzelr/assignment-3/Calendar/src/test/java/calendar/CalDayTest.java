...
public class CalDayTest {
	...
	*/
    ...
    int thisMonth = today.get(Calendar.MONTH)+1;
		...
		//assertions
		assertEquals(list, calDay.getAppts());
	...
	*/
    ...
    int thisMonth = today.get(Calendar.MONTH)+1;
		...
		//this statement is correct, but it trips of a bug and thus cannot run with PITest
		//assertEquals("\t --- " + thisMonth + "/" + thisDay + "/" + thisYear + " --- \n --- -------- Appointments ------------ --- \n", calDay.toString());
		//meanwhile this is wrong
		comp = calDay.toString();
		assertEquals("\t --- " + thisMonth + "/" + thisMonth + "/" + thisYear + " --- \n --- -------- Appointments ------------ --- \n\n", comp);
	}
